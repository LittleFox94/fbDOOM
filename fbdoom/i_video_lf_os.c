...
struct fb_var_screeninfo {
  uint16_t xres;
  uint16_t yres;
  uint16_t xres_virtual;
  uint16_t yres_virtual;
  uint8_t  bits_per_pixel;
  bool     grayscale;
  struct fb_var_color red;
  struct fb_var_color green;
  struct fb_var_color blue;
  struct fb_var_color transp;
};
...
void I_InitGraphics (void)
{
    ...
    sc_do_hardware_framebuffer(&I_VideoBuffer_FB, &fb.xres, &fb.yres, &fb.xres_virtual, &fb.bits_per_pixel);
	...
	I_VideoBuffer = (byte*)malloc(fb.xres * fb.yres * (fb.bits_per_pixel/8));     // For a single write() syscall to fbdev
